...
{
    public class ProductSerVice : BaseImplement<Product>, IProductSerVice
    {
       
        private readonly IMapper _mapper;
        private readonly ApplicationDbContext _context;
        public ProductSerVice(ApplicationDbContext context, IMapper mapper):base(context)
        {
            _mapper = mapper;
            _context = context;
        }
        ...
        public async Task<List<ProductViewModel>> GetAllProduct()
        {
            ...
            var listproductViewModel = _mapper.Map<List<Product>, List<ProductViewModel>>(listproduct);
        ...
        public async Task<PagedResult<ProductViewModel>> GetAllProductPaging(PageRequest request)
        {
            ...
            var listproductViewModel = _mapper.Map<List<Product>, List<ProductViewModel>>(listproduct);
            var product = from p in listproductViewModel
                          select new { p };
        ...
        public async Task<PagedResult<ProductViewModel>> GetAllProductPagingPart2(RequestBase request)
        {
            ...
            {
                product = product.Where(x => x.p.LastPrice > request.FromPrice && x.p.LastPrice < request.ToPrice);
            }
            var total = product.Count();
            var vt = product.Skip((request.PageIndex - 1) * (request.PageSize)).Take(request.PageSize)
                .Select(x => new ProductViewModel()
